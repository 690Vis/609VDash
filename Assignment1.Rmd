...
plot <- ggplot(df, aes(x = County)) + # x axis is the Counties
  ...
  scale_y_continuous(sec.axis = sec_axis(trans = ~.*0.001298786, # creating the second y-axis with the correct scaling
                                         name = "Cumulative (%)", # name of second y-axis
                                         breaks = seq(0, 100, by = 25))) + # how the axis numbers will be shown
  labs(title = "Pareto Plot, Reduced Lunch by County", # title
       subtitle = "How many students receive reduced lunch in each county of Wyoming", # subtitle
